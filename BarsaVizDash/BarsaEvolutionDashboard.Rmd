---
title: "PRA 2 - Visualització"
author: "Albert Geli Taberner"
...
```{r}
season_stats <- read.csv("season_stats.csv"); season_stats
tactics_formation <- read.csv("tactics_formation.csv"); tactics_formation
player_stats <- read.csv("player_stats.csv", quote = ""); player_stats
```
...
Creo la gràfica de Season Stats:
...
Creo la gràfica *pie chart* de formacions:
...
Creo a mode d'exemple, una visualització de les estadístiques de jugadors (exemple amb 2008/2009) amb la qual després crearé la gràfica interactiva final en el dashboard:

```{r}
players89 <- plot_ly() %>%

  add_trace(data = player_stats[player_stats$Season == "2008/2009", ], x = ~player.name, y = ~Goals,
            type = 'bar', name = "Goals", visible = TRUE) %>%
  add_trace(data = player_stats[player_stats$Season == "2008/2009", ], x = ~player.name, y = ~xG,
            type = 'bar', name = "xG", visible = TRUE) %>%
  add_trace(data = player_stats[player_stats$Season == "2008/2009", ], x = ~player.name, y = ~key_passes,
            type = 'bar', name = "Key Passes", visible = TRUE) %>%
  add_trace(data = player_stats[player_stats$Season == "2008/2009", ], x = ~player.name, y = ~through_ball,
            type = 'bar', name = "Through Ball", visible = TRUE) %>%
  add_trace(data = player_stats[player_stats$Season == "2008/2009", ], x = ~player.name, y = ~assists,
            type = 'bar', name = "Assists", visible = TRUE) %>%
...
**Anem amb el Dashboard**

```{r}
seasons <- season_stats$Season; seasons
```
